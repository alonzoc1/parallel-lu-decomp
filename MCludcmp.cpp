...
void MCludcmp(MatrixBase<Derived> &a, long n, int *indx) {
    // Create a structure to hold both value and index
struct {
    double value;
    int index;
    ...
    // mpi parameters
    int comm_sz, my_rank;
    int local_n, start_col, end_col, remain;
    ...
    MPI_Bcast(scaling, n, MPI_DOUBLE, 0, MPI_COMM_WORLD); // bcast scaling

    // main crout's loop. iterate by column. Use MPI to divide columns
    remain = n % comm_sz; // check if n  is multiple of comm_sz
    local_n = n / comm_sz;   // divide columns
    start_col = my_rank * local_n;

    // add extra column if there are remainders
    if (my_rank < remain) {
        local_n++;
        start_col += my_rank;
    } else
...
#pragma omp parallel for default(none) private(i, sum, k)                      \
    shared(a, j, end_col, big, imax, cout) schedule(static)
        for (i = j; i < end_col; i++) {
            sum = a(i, j);
            for (k = 0; k < j; k++)
                sum -= a(i, k) * a(k, j);
        ...
        if (j != end_col) {
            dum = (1.0 / a(j, j));
#pragma omp parallel for default(none) private(i, k) shared(j, end_col, a, dum)      \
    schedule(static)
            for (i = j + 1; i < end_col; i++)
                a(i, j) *= dum;
        }
    }
    MPI_Barrier(MPI_COMM_WORLD);
